...
class EvalDataset(Dataset):
    ...
    def __init__(self,
        ...
        # with open(os.path.join(self.data_root, name, 'ns_all.pkl'), 'rb') as fp:
        #     self.uindex2negatives_test = pickle.load(fp)
        with open(os.path.join(self.data_root, name, 'df_rows.pkl'), 'rb') as fp:
            self.df_rows = pickle.load(fp)
    ...
    def __getitem__(self, index):
        ...
        if self.target == 'valid' or self.target =='test':
            negative_tokens = set()
        ...
        '''
        if self.target == 'valid' or self.target =='test':
            all_iindices = list(range(1, self.num_items+1))
            seen_iindices = list(set(self.df_rows[self.df_rows['uindex'] == uindex]['iindex']))
            seen_mask = np.isin(all_iindices, list(seen_iindices))
            negative_tokens = all_iindices * (~seen_mask)
