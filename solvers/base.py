...
class BaseSolver:
    ...
    def train_one_epoch(self, epoch: int) -> None:
        ...
        for batch in pbar:
            ...
            torch.cuda.synchronize()  # Sync CUDA ops for accurate timing
            ...
            # allocated_mem = torch.cuda.memory_allocated(device) / 1024 ** 2  # MB
            # cached_mem = torch.cuda.memory_reserved(device) / 1024 ** 2  # MB
            # peak = torch.cuda.max_memory_allocated() / (1024 ** 2)   # peak memory

            # print('============================================================')
            # print(
            # f"Batch {batch_idx}: "
            # f"Time = {elapsed_time:.4f}s | "
            # f"Allocated GPU Memory = {allocated_mem:.2f}MB | "
            # f"Cached GPU Memory = {cached_mem:.2f}MB | "
            # f"Peak GPU Memory = {peak:.2f}MB")
    ...
    def evaluate_valid(self, epoch: int) -> None:
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                #print('len of rankers: ', len(rankers))
                # evaluate
